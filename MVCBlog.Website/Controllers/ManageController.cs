...
namespace MVCBlog.Website.Controllers
{
    ...
    public partial class ManageController : Controller
    {
        ...
        public virtual async Task<ActionResult> Index(ManageMessageId? message)
        {
            ...
            var services = db.ServiceUsers.Where(_ => _.EndDate == null).Include(s => s.Service).Where(_ => _.AspNetUserId == userId).Select(a => a.Service).Include(_=>_.ServiceType).ToList();
            var monthlyFees = db.SocietyMonthlyFees.Where(_ => _.AspNetUserId == userId && _.IsClosed).ToList();
        ...
        {
            ViewBag.StatusMessage =
                message == ManageMessageId.RemoveLoginSuccess ? "El inicio de sesión externo fue eliminado."
                : message == ManageMessageId.Error ? "Se ha producido un error."
                : "";
            var user = await UserManager.FindByIdAsync(User.Identity.GetUserId());
            if (user == null)
