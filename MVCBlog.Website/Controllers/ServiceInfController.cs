...
namespace MVCBlog.Website.Controllers
{
    ...
    public partial class ServiceInfController : Controller
    {
        private readonly IRepository repository;
        private string nameInfo = Common.Info + "_" + Common.AspNetUser;

        public ServiceInfController(IRepository repository)
        {
        ...
        {
            ServiceInfViewModels viewModel = new ServiceInfViewModels();

            return View(viewModel);
        }

        [HttpPost]
        public virtual JsonResult Export(ServiceInfViewModels viewModel)
        {
            JsonResult result = new JsonResult();
            switch (viewModel.InfList)
            {
                case ServiceInf.ServiceForType:
                    result = SocInfGet();
                    break;
                case ServiceInf.ServiceForUsersForCoach:
                    result = SocInfGet();
                    break;
            }
            return result;
